...
/* Fix: Changed Relm to Realm to match the exported member in types.ts */
import { Realm } from '../types';

interface RelmSettingsModalProps {
  relm: Realm;
  onClose: () => void;
  onUpdate: (relmId: string, updates: Partial<Realm>) => void;
}

export const RelmSettingsModal: React.FC<RelmSettingsModalProps> = ({ relm, onClose, onUpdate }) => {
  const [name, setName] = useState(relm.name);
  const [bio, setBio] = useState(relm.bio || '');
  const [avatarUrl, setAvatarUrl] = useState(relm.avatarUrl || '');
  const [bannerColor, setBannerColor] = useState(relm.bannerColor || '#5865f2');
  ...
  return (
    <div className="fixed inset-0 z-[600] flex items-center justify-center bg-black/80 backdrop-blur-sm animate-in fade-in duration-300" onClick={onClose}>
      ...
      >
        <div className="relative h-40 flex items-center justify-center" style={{ backgroundColor: bannerColor }}>
          ...
          >
            {avatarUrl ? (
              ...
              <span className="text-3xl font-black text-white">{relm.initials}</span>
            )}
        ...
        <div className="p-8">
          <h2 className="text-2xl font-black text-white uppercase tracking-tighter mb-8">Relm Settings</h2>
          ...
          <div className="space-y-6">
            ...
            <div>
              <label className="block text-[10px] font-black text-neutral-500 uppercase tracking-widest mb-2">Relm Bio</label>
              <textarea 
                value={bio} 
